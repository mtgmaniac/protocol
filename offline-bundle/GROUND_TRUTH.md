...
7. **Freeze** — SUPERSEDED: this paragraph described the dead bank/thaw model. **FREEZE = REPEAT, per Kev 2026-07-06, FINAL** (see `docs/TRUTH.md` rule 7 and `docs/DECISIONS_RESOLVED.md` #1 for the ruling and the full bank/thaw → lockout → repeat lineage): the frozen die crusts static in the tray, keeps its face on its next N rolls, and its unit acts again on that same result each time before thawing. Do NOT re-implement the banked-face or lockout readings from this file.
