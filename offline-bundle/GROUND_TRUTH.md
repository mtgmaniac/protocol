...
Chip doctrine: card chips are ONLY Burn / Mark / ±Roll / Ward (cap 3, +N overflow badge opens inspect). Cloak = ghosted portrait · Freeze/Petrify = die crust (ice cyan / stone gray) · Jam = die tint + "JAM ≤N" marker · Rewrite/Hijack = pending marker on the die + readout entry · Spike = readout pip only · Taunt/Lure = incoming target-intent markers — the rust "◎N" corner badge on the card is the canonical targeting read ("◎N" enemies aiming here / "◎!" lured, on both ends); it resolves through the long-press unit inspect with a name + one-line definition, and there is no separate intent-line system. Detonate pip shows the live burst when the target is known. Nat-20 signature = gold wash + shake + overload stinger + heavier hit-pause + ability-name slam. Keyword feedback table lives in offline-bundle/ANIMATION.md. Dice numerals render near-white in the engraved pipeline. Summon layout floor: cards never shrink below the 3-unit slot width (rows scroll instead).
