...
**Autoload convention for `-s` tests (Kev 2026-09-25, after the fourth incident):** files exercised by headless `-s` tests resolve autoloads through `get_node("/root/…")`, never the bare global identifier (`AudioManager`, `GameState`, …), because `-s` compiles the entry script before autoloads register — a bare identifier anywhere in its compile-time closure (`preload(...)` chains and `extends "res://…"` / `extends ClassName` parents) is a compile error that cascades up the chain and hangs the test to its timeout. Incidents: `operation_lore_presentation_test.gd` (now run by .tscn), `training_prompt.gd` via `training_flow` (2026-09-25). Scripts loaded later at runtime compile after autoloads exist, which is why **`battle_scene.gd` (53 bare autoload references, 3 `get_node`, counted 2026-09-25) is safe only because no `-s` test preloads it — convert it before any headless `-s` test preloads or extends it.** Enforced by the `autoload closure` gate (`scripts/checks/autoload_closure.py`), which re-injects the known incidents on every run and fails if any goes undetected.
...
**Squad selection polish (G-16, 2026-09-08):** Locked encounters with no focused hero retain a transparent detail-panel footprint, so first selection does not move the grid. Locked hero labels are blank with reserved height; silhouettes and LOCKED remain. Unplayed encounters omit NO CLEARANCE; real progress and lock status remain. Briefing headings and keys stay uppercase; site/situation/objective descriptions use sentence case and proper boss names. Training uses SCRAP for its displayed enemy identity and introduces battle two as learning new abilities.
