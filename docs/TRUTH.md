...
- **Operation lore presentation:** an operation newly awarded at run end shows its one-time accepted origin acknowledgement. Its first later run stops on a DEPLOY slate; later runs show the same slate but auto-advance after **2.25 seconds** and remain immediately tappable. Battle 10 loads the battlefield first, then requires ENGAGE on a boss alert whose standing-rule mechanic is sourced from `CombatManager.BOSS_STANDING_RULES`; after acknowledgement, a compact tappable rule-name reminder replaces the alert. These first-view flags live in `SaveManager.onboarding`; old saves mark already-unlocked origins seen and infer prior deployment only from `best_clear_by_op`.
