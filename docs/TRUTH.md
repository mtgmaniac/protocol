...
**Hero portraits:** base art `assets/portraits/<hero_id>.png` (`DataManager.HERO_PORTRAIT_BY_ID`). **Evolved art convention: `assets/portraits/<hero_id>_<evo_id>.png`**, where `evo_id` is the evolution entry's `id` field in `heroes.data.json` — the lowercased callsign, schema-enforced (pyro, arc, blade, ravager, bulwark, sentinel, glacier, trench, medic, synth, overclocked, phantom, shadow, wraith, noise, nullwire). `DataManager.get_evolution_portrait()` resolves it; a missing file silently falls back to the base portrait (never errors, never blanks). The swap rides `GameState.get_run_unit_data()`, so every run-unit surface (battle cards, equip labels, sim) inherits it; the evolution screen previews each branch's own portrait. All 24 files (8 base overwrites + 16 evolutions) installed 2026-07-07. Both art styles pass through the crop-to-content contract (`_crop_to_content`, cutout vs full-bleed tag). **Zoom-normalized 2026-07-12 (Batch 6, three passes — the third one is the load-bearing one):** the source art was authored at inconsistent subject zooms (some busts ~20% larger than others), so every hero portrait PNG (25 files incl. variants) was re-cropped OFFLINE to a common helmet size + top position (auto baseline + ~12 per-file hand-tuned zoom multipliers; cropping only, native-res). **Critically, the runtime mat-crop was the real culprit:** `DataManager._crop_to_content(matted_bust=true)` re-cropped every hero portrait at load to its own silhouette bbox, re-deriving zoom from per-subject proportions and silently undoing any offline framing. Hero portraits now load through `_finalize_hero_portrait` — opaque (normalized) art is returned WHOLE, tagged cutout (`full_bleed=false`, so square frames still show the face and battle cards add the uniform `PORTRAIT_TOP_PAD`); only art with real alpha still crops to its opaque bbox. Enemy portraits keep the old pipeline untouched. Verified numerically: rendered helmet widths across the squad tiles measure pixel-identical (111/111/111). **The offline-normalized PNG is the framing authority — never reintroduce a runtime content-crop for hero portraits.**
