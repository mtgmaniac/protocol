...
func _run_capture() -> void:
	...
	var tray: Node = current_scene.get_node("%DiceTray3D")
	var statuses := [{"frozen": true}, {"frozen": true, "flavor": "petrify"}, {"jam_cap": 10, "rewrite": true}]
	var views: Array = current_scene.get("hero_card_views")
	for i in views.size():
		var id: String = str(views[i]["state"]["id"])
		tray.call("set_die_status", "hero", id, statuses[i])
...
func _parse_args() -> Dictionary:
	...
	for arg in OS.get_cmdline_args():
		if arg.begins_with("--capture-output="):
			...
			# Simulate device safe-area insets (top,bottom design px) on desktop â€”
			...
			# Routed through apply_inset_trims like a real Android device.
		...
		elif arg.begins_with("--capture-insets-raw="):
			# Same simulation, but BYPASSES the trims and hides the bezel strip â€”
			# reproduces the pre-Build-J presentation for before/after comparisons.
			var raw_parts: PackedStringArray = arg.get_slice("=", 1).split(",", false)
			if raw_parts.size() >= 2:
...
func _prepare_run(config: Dictionary) -> void:
	...
	if config.has("inset_top"):
		# Simulated device insets: poke the single source of truth, PIN it so
		# the desktop refresh can't wipe it (the pre-Build-J flag silently
		# captured zero-inset layouts â€” root size_changed re-measured the real
		# desktop after injection), then have the header re-apply so the sim
		# matches the real device pipeline. The normal mode routes through
		# apply_inset_trims as an Android device would; --capture-insets-raw
		# bypasses trims and hides the bezel strip (pre-Build-J presentation).
		PixelUI.sim_insets_pinned = true
		if bool(config.get("insets_raw", false)):
			PixelUI.safe_top = int(config["inset_top"])
			PixelUI.safe_bottom = maxi(int(config["inset_bottom"]), PixelUI.SAFE_BOTTOM_RESERVE)
		else:
			var trimmed_insets: Vector2i = PixelUI.apply_inset_trims(
				int(config["inset_top"]),
				maxi(int(config["inset_bottom"]), PixelUI.SAFE_BOTTOM_RESERVE),
				"Android")
			PixelUI.safe_top = trimmed_insets.x
			PixelUI.safe_bottom = trimmed_insets.y
		...
		if bool(config.get("insets_raw", false)):
			var bezel: Variant = header.get("_bezel_bottom") if header != null else null
...
func _wait_for_battle_scene(config: Dictionary) -> void:
	...
	if item_target != "":
		...
		# targeting â€” should cancel the item back to the loadout, same as tapping the card.
	...
	if bool(config.get("loadout_tap", false)):
		...
		if bool(config.get("loadout_poor", false)) and current_scene != null and "protocol_points" in current_scene:
			# Drain Protocol so the tapped item is unaffordable â€” exercises the red-flash /
...
func _open_loadout() -> void:
	...
	if "protocol_points" in current_scene:
		current_scene.set("protocol_points", 10)
	# The item list lives on the protocol module â€” refresh it AFTER seeding so
...
func _tap_loadout_first_item() -> void:
	...
	# needs real OS input), so drive the menu's row handler directly â€” this still exercises
...
func _force_enemy_rolls_shield_scenario() -> void:
	...
	# Force roll to 1 â€” scrap_drone's recharge (shield 8, 2t) sits in the low
