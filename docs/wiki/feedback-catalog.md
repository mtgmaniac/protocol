...
| **Status chip row** | `_build_compact_status_tokens` | EVERY active status gets a chip (Kev 2026-07-10 — the old six-type doctrine is lifted): Burn · Shield · Mark · ±Roll (green die = +roll) · Firewall · Taunt · Cloak · Jam · Rewrite · Spike. The ROW visually caps at `STATUS_MAX_VISIBLE` and overflows into a `+N` badge — the full list lives in the unit long-press. All state-driven |
