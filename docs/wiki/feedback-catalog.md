...
> **Code-verified truth as of `6d9118c` (2026-07-08). Reference for UI/icon/audio work (Prompts 6, 7, 8).**
...
| Mark applied | float `◎ MARKED` (`:343`) + `MARK` chip | `mark` event / `marked` state |
| Mark consumed | (no dedicated pip) | `marked` cleared in `combat_manager` on the next real hit — chip disappears state-driven |
| Ward consume (firewall) | `_hex_flash:522-546` (`:447-450`) | `block` event with `amount <= 0` → cyan hexagon pop + `✕ NEGATED` float |
