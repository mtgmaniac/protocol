...
func _ready() -> void:
	...
	if hp_label:
		...
		hp_label.mouse_entered.connect(_on_element_mouse_entered.bind(TOOLTIP_MAP.HP))
	...
	if xp_bar:
		...
		xp_bar.mouse_entered.connect(_on_element_mouse_entered.bind(TOOLTIP_MAP.XP))
...
func _on_element_mouse_entered(text: String) -> void:
	_hovered_element_text = text
	# On desktop, show immediately. On mobile, we rely on long press.
...
func _show_tooltip(text: String = "") -> void:
	...
	var tooltip_label: Label = get_node_or_null("%TooltipLabel")
	if tooltip_label:
...
func _hide_tooltip() -> void:
	var tooltip_panel: PanelContainer = get_node_or_null("TooltipPanel")
	if tooltip_panel:
...
func _populate_badges(container: Container, values: Array, empty_text: String) -> void:
	...
	for value in values:
		...
		badge.mouse_entered.connect(_on_element_mouse_entered.bind(description))
...
func _populate_dice_chart(container: VBoxContainer, rows: Array, active_zone: String) -> void:
	...
	for row_variant in rows:
		...
		range_tab.mouse_entered.connect(_on_element_mouse_entered.bind(range_tooltip))
	...
	name_label.mouse_entered.connect(_on_element_mouse_entered.bind(ability_desc))
	name_label.mouse_exited.connect(_on_element_mouse_exited)
...
func _add_effect_chips_to_flow(chip_flow: HFlowContainer, chips: Array, chip_size: Vector2, icon_size: Vector2, font_size: int) -> void:
	...
	for chip_variant in chips:
		...
		chip_panel.mouse_entered.connect(_on_element_mouse_entered.bind(tooltip_text))
