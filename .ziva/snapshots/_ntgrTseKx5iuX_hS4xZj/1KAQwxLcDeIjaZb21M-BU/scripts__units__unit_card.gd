...
func setup_card(display_name: String, current_hp: int, max_hp: int, dice_result: String, ability_text: String, target_text: String, status_list: Array, _gear_list: Array, xp_ratio: float, is_dead: bool, accent_color: Color, portrait_texture: Texture2D = null, tooltip_text: String = "", ability_chart_rows: Array = [], active_zone: String = "") -> void:
	var portrait: TextureRect = get_node("CardFrame/Margin/VBox/ContentRow/PortraitFrame/Portrait")
	var portrait_frame: PanelContainer = get_node("CardFrame/Margin/VBox/ContentRow/PortraitFrame")
	var name_label: Label = get_node("CardFrame/Margin/VBox/NameLabel")
	var hp_bar: ProgressBar = _resolve_hp_bar()
	var hp_label: Label = _resolve_hp_label(hp_bar)
	var target_label: Label = get_node("CardFrame/Margin/VBox/TargetLabel")
	var status_title: Label = get_node("CardFrame/Margin/VBox/ContentRow/InfoColumn/StatusTitle")
	var gear_title: Label = get_node("CardFrame/Margin/VBox/ContentRow/InfoColumn/GearTitle")
	var status_effects: Container = get_node("CardFrame/Margin/VBox/ContentRow/InfoColumn/StatusEffects")
	var gear_slots: Container = get_node("CardFrame/Margin/VBox/ContentRow/InfoColumn/GearSlots")
	var xp_bar: ProgressBar = get_node("CardFrame/Margin/VBox/ContentRow/InfoColumn/XPBar")
	...
	if hp_bar != null and hp_label != null:
		_place_hp_row_above_target(hp_bar, target_label)
...
func _resolve_hp_bar() -> ProgressBar:
	...
	if hp_bar != null:
		return hp_bar
	return get_node_or_null("CardFrame/Margin/VBox/ContentRow/InfoColumn/HPRow/HPBar") as ProgressBar
...
func _resolve_hp_label(hp_bar: ProgressBar) -> Label:
	...
	if hp_label != null:
		return hp_label
	hp_label = get_node_or_null("CardFrame/Margin/VBox/ContentRow/InfoColumn/HPLabel") as Label
	if hp_label != null:
		return hp_label
	if hp_bar != null:
...
func _get_icon_texture(icon_kind: String, tint: Color) -> Texture2D:
	var cache_key: String = "%s_%s" % [icon_kind, tint.to_html()]
	...
	match icon_kind:
		"damage":
			_draw_damage_icon(image, tint)
		"shield":
			_draw_shield_icon(image, tint)
		"heal":
			_draw_heal_icon(image, tint)
		"dot":
			_draw_dot_icon(image, tint)
		"dice":
			_draw_dice_icon(image, tint)
		"frost":
			_draw_frost_icon(image, tint)
...
func _paint_points(image: Image, points: Array[Vector2i], color: Color, block_size: int) -> void:
	for point in points:
		_paint_rect(image, Rect2i(point, Vector2i(block_size, block_size)), color)
		...
		for x in range(0, image.get_width()):
			...
			for neighbor in neighbors:
				...
				if image.get_pixelv(neighbor).a <= 0.0:
					outline_points.append(neighbor)
	for point in outline_points:
		image.set_pixelv(point, color)


func _draw_damage_icon(image: Image, tint: Color) -> void:
	var points: Array[Vector2i] = [
		Vector2i(12, 1), Vector2i(11, 2), Vector2i(10, 3), Vector2i(9, 4),
		Vector2i(8, 5), Vector2i(7, 6), Vector2i(6, 7), Vector2i(5, 8),
		Vector2i(9, 8), Vector2i(10, 8), Vector2i(11, 8), Vector2i(10, 9),
		Vector2i(9, 10), Vector2i(8, 11), Vector2i(7, 12), Vector2i(6, 13),
		Vector2i(5, 14), Vector2i(4, 15), Vector2i(8, 14), Vector2i(9, 13),
		Vector2i(10, 12), Vector2i(11, 11), Vector2i(12, 10), Vector2i(13, 9),
		Vector2i(14, 8), Vector2i(15, 7), Vector2i(16, 6)
	]
	_paint_points(image, points, Color(0.05, 0.02, 0.02, 0.9), 2)
	_paint_points(image, points, tint, 1)


func _draw_shield_icon(image: Image, tint: Color) -> void:
	_paint_rect(image, Rect2i(7, 3, 10, 3), tint)
	_paint_rect(image, Rect2i(5, 6, 14, 4), tint)
	_paint_rect(image, Rect2i(6, 10, 12, 3), tint)
	_paint_rect(image, Rect2i(8, 13, 8, 3), tint)
	_paint_rect(image, Rect2i(10, 16, 4, 3), tint)
	_paint_rect(image, Rect2i(11, 19, 2, 2), tint)
	_paint_outline(image, Color(0.02, 0.04, 0.08, 0.95))


func _draw_heal_icon(image: Image, tint: Color) -> void:
	_paint_rect(image, Rect2i(10, 3, 4, 18), Color(0.02, 0.08, 0.04, 0.95))
	_paint_rect(image, Rect2i(3, 10, 18, 4), Color(0.02, 0.08, 0.04, 0.95))
	_paint_rect(image, Rect2i(11, 4, 2, 16), tint)
	_paint_rect(image, Rect2i(4, 11, 16, 2), tint)


func _draw_dot_icon(image: Image, tint: Color) -> void:
	_paint_rect(image, Rect2i(7, 3, 10, 3), tint)
	_paint_rect(image, Rect2i(5, 6, 14, 8), tint)
	_paint_rect(image, Rect2i(7, 14, 10, 3), tint)
	_paint_rect(image, Rect2i(8, 17, 2, 3), tint)
	_paint_rect(image, Rect2i(14, 17, 2, 3), tint)
	_paint_rect(image, Rect2i(7, 8, 4, 4), Color(0.025, 0.015, 0.04, 1.0))
	_paint_rect(image, Rect2i(13, 8, 4, 4), Color(0.025, 0.015, 0.04, 1.0))
	_paint_rect(image, Rect2i(11, 12, 2, 2), Color(0.025, 0.015, 0.04, 1.0))
	_paint_rect(image, Rect2i(4, 19, 5, 2), tint)
	_paint_rect(image, Rect2i(15, 19, 5, 2), tint)
	_paint_outline(image, Color(0.04, 0.01, 0.06, 0.95))


func _draw_dice_icon(image: Image, tint: Color) -> void:
	_paint_rect(image, Rect2i(4, 4, 16, 16), tint)
	_paint_rect(image, Rect2i(6, 6, 12, 12), Color(0.08, 0.11, 0.15, 1.0))
	_paint_rect(image, Rect2i(7, 7, 3, 3), tint)
	_paint_rect(image, Rect2i(14, 7, 3, 3), tint)
	_paint_rect(image, Rect2i(7, 11, 3, 3), tint)
	_paint_rect(image, Rect2i(14, 11, 3, 3), tint)
	_paint_rect(image, Rect2i(7, 15, 3, 3), tint)
	_paint_rect(image, Rect2i(14, 15, 3, 3), tint)
	_paint_outline(image, Color(0.04, 0.03, 0.01, 0.95))


func _draw_frost_icon(image: Image, tint: Color) -> void:
	for y in range(2, 14):
		image.set_pixel(7, y, tint)
		image.set_pixel(8, y, tint)
	for x in range(2, 14):
		image.set_pixel(x, 7, tint)
		image.set_pixel(x, 8, tint)
	for offset in range(3, 13):
		image.set_pixel(offset, offset, tint)
		image.set_pixel(15 - offset, offset, tint)
