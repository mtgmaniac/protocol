...
const ICON_DOT := "\u2620"
const ICON_MAP = {
	"damage": preload("res://assets/generated/icon_damage_v2_frame_0_1776040040.png"),
	"shield": preload("res://assets/generated/icon_shield_v2_frame_0_1776040039.png"),
	"heal": preload("res://assets/generated/icon_heal_v2_frame_0_1776040039.png"),
	"dot": preload("res://assets/generated/icon_dot_v2_frame_0_1776040042.png"),
	"dice": preload("res://assets/generated/icon_dice_v2_frame_0_1776040041.png"),
...
const TOOLTIP_MAP = {
	...
	"POI": "Poison: Deals damage at the start of every turn. Ignores shields.",
	...
	"DMG": "Damage: Deals direct damage to the target's HP.",
	...
	"HP": "Health Points: If this reaches zero, the unit is defeated.",
...
func _ready() -> void:
	...
	# Connect existing UI elements for tooltips
	...
	if hp_label:
		...
		if not hp_label.mouse_entered.is_connected(_on_element_mouse_entered):
			hp_label.mouse_entered.connect(_on_element_mouse_entered.bind(TOOLTIP_MAP.HP, hp_label))
			hp_label.mouse_exited.connect(_on_element_mouse_exited)
...
func _on_hold_timer_timeout() -> void:
	if not _hovered_element_text.is_empty():
...
func _populate_badges(container: Container, values: Array, empty_text: String) -> void:
	...
	for value in values:
		...
		badge.custom_minimum_size = Vector2(80, 32)
		...
		badge.gui_input.connect(_on_badge_gui_input)
...
func setup_card(display_name: String, current_hp: int, max_hp: int, dice_result: String, ability_text: String, target_text: String, status_list: Array, _gear_list: Array, xp_ratio: float, is_dead: bool, accent_color: Color, portrait_texture: Texture2D = null, tooltip_text: String = "", ability_chart_rows: Array = [], active_zone: String = "") -> void:
	...
	if portrait_texture != null:
		var tex_ratio = float(portrait_texture.get_width()) / float(portrait_texture.get_height())
		portrait_aspect.ratio = tex_ratio
		# Match frame style
...
func _get_tooltip_for_chip(chip: Dictionary, icon_kind: String) -> String:
	...
	if icon_kind.to_upper() == "DAMAGE":
		return "Damage: Deals %s direct damage." % value
	if icon_kind.to_upper() == "SHIELD":
		return "Shield: Provides %s protection." % value
	if icon_kind.to_upper() == "HEAL":
		return "Heal: Restores %s health." % value
	if icon_kind.to_upper() == "DOT":
		return "Poison: Inflicts %s damage over time." % value
	if icon_kind.to_upper() == "DICE":
		return "Dice: Modifies roll by %s." % value
...
func _populate_dice_chart(container: VBoxContainer, rows: Array, active_zone: String) -> void:
	...
	var row_ids = []
	for r in rows:
		row_ids.append(str(r.get("range_text", "")) + str(r.get("ability_name", "")) + str(r.get("zone", "")))
	...
	for row_variant in rows:
		...
		# Add tooltip for the range tab itself
		range_tab.mouse_filter = Control.MOUSE_FILTER_STOP
		var range_tooltip = "Tier Range: " + str(row_data.get("range_text", ""))
		range_tab.mouse_entered.connect(_on_element_mouse_entered.bind(range_tooltip, range_tab))
		range_tab.mouse_exited.connect(_on_element_mouse_exited)
	...
	var ability_name_label: Label = Label.new()
	ability_name_label.text = str(active_row.get("ability_name", "Ability"))
	ability_name_label.clip_text = true
	ability_name_label.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
	ability_name_label.custom_minimum_size = Vector2(112, 0)
	ability_name_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	ability_name_label.mouse_filter = Control.MOUSE_FILTER_STOP
	PixelUI.style_label(ability_name_label, 26, PixelUI.TEXT_PRIMARY, 3)
	detail_row.add_child(ability_name_label)
	
	# Add tooltip for ability name
	var ability_desc = str(active_row.get("description", "A special protocol ability."))
	ability_name_label.mouse_entered.connect(_on_element_mouse_entered.bind(ability_desc, ability_name_label))
	ability_name_label.mouse_exited.connect(_on_element_mouse_exited)
...
func _make_range_tab(row_data: Dictionary, is_active_row: bool) -> Label:
	...
	range_label.mouse_filter = Control.MOUSE_FILTER_STOP
	range_label.mouse_entered.connect(_on_element_mouse_entered.bind(tooltip_text, range_label))
	range_label.mouse_exited.connect(_on_element_mouse_exited)
	range_label.gui_input.connect(_on_badge_gui_input)
