...
| Item | Status | Scope |
|------|--------|-------|
| **Tutorial welcome-fit + mark-out delta (Jul 2026, Prompt 6)** | **Done** | Coach panel sizes to its text: measured m5x7 bounds + named paddings, balanced wrap for multi-line copy (WELCOME shrinks ~40%, aftermath wraps evenly), centered whole-px placement, bottom anchor clears safe_bottom — applies to tutorial AND primer coachmarks (shared SpotlightLayer). Rig v2.3: Strike rolls 9 (Suppression Fire 6) — Target Lock/mark OUT of the drill, taught by its primer at first real sighting; T1 = 6+11 = 17 (same 35→18 checkpoint), order-INVARIANT (audit-asserted under reversed order, replacing mark-persistence regressions); smoke bans mark alongside leech on all rigged bands. Copy: beats 6/8/9/14 updated, no mark/+50% language anywhere. Still 24 beats, 250/0, before/after captures. |
| **Tutorial item-signpost delta (Jul 2026, Prompt 5)** | **Done** | Rig v2.2: Engineer rolls 12 (Overdrive) BOTH turns — Barrier Deploy leaves the rig; the T2 kill closes on dice alone (10+11=21 into 18). Shock Charge grant + tutorial `items_free` DELETED (the item-use beat was only survivable via a fiction that mis-taught the economy); the item lesson is a tap-through SIGNPOST after the band-jump beat stating the real cost (1 Protocol, engine-verified). Smoke: dice-only kill checkpoints, empty-loadout assert, Nudge-at-0-PP rejection replaces the double-Nudge case. `item_used` emission kept as unconsumed plumbing. Still 24 beats; audit 250/0. |
| **Tutorial playtest fix pass (Jul 2026, Pixel)** | **Done** | Ten findings: rig v2.1 (mark payoff to Engineer — leech and the nat-20 slam stay out of the drill; Medic shields T1 with Diagnostic Pulse; Protocol T2 = exactly 1), badge beat DELETED (24 steps), Skip button DELETED (opt-in + replayable + un-strandable; header back remains), beat-3 drone-only intro, two-stage assign spotlights (holes move to legal targets on `targeting_started` — smoke-asserted), telegraph beat = drone card+pip+die, band-jump adds the die, coach hidden until placed + bounded layout-retry (kills the placement blip generically). All copy re-verified against the engine (mark 11→17, drone 35→18→dead; shield soaks 3). |
| **Tutorial rebuild v2 (Jul 2026)** | **Done** | Honest rig: real 35-HP Scrap Drone (10-HP override deleted), rigged dice only (T1 3/2/13, T2 8/12/6, drone 6 = Stab 7), one granted Shock Charge. 25-beat script teaching cast order (mark→spend), shield soak, band-jump Nudge, and the item action; status-chip teaching delegated to primers (suppression verified to never write `primers_seen`). Gate schema gained the `hero` payload predicate + `item_used` event; `die:<unit_id>`/`item` spotlight keys. Dice-settle rig: tray receives values pre-roll, settle presents the rigged face (post-settle repaint snap deleted). Stall-proofed: tutorial items cost 0, resequenced mark + double-Nudge + drained pool all still win (audit-pinned, 5 regressions; smoke drives happy + stall-proof scenarios). |
| **Player-chosen cast order (Jul 2026, Model A)** | **Done** | Assignment order = firing order. `cast_stamp` on hero state (autos stamp in squad order at targeting start, manual picks at assignment); re-tap unassigns/recommits at end of order (the only reorder mechanism); resolution sorts a view — `_hero_states` stays squad order (L2's old `set_hero_order` array reorder replaced by stamp writing, fixing its enemy-targeting leak). Card badge (portrait top-left), "Cast order:" round log, `cast_order` in JSONL, L2 `order_mode` A/B seam (search/setups/squad). **Tutorial redesign around the mechanic DEFERRED — tracked separately; the 23-step script passes unchanged.** |
