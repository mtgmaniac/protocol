...
func _make_grid_cell(entry: Dictionary) -> Control:
	...
	if item != null and item.icon != null:
		# Integer icon law: whole multiples of native only; low-res art rides
		# the emblem plate inside make_integer_icon.
		var icon := PixelUI.make_integer_icon(item.icon, GRID_ICON_BOX, PixelUI.DT_AMBER)
...
# A newly-awarded operation shows its one-time accepted origin acknowledgement
# here (moved from the run-end screen with the old UNLOCKED panel — this screen
# is now where the operation is announced).
