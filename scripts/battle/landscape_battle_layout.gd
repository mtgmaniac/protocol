...
func refresh() -> void:
	...
	var card_w: float = floorf(minf(card_h * 0.80, _scene.board.size.x * 0.19) * pixel_scale.x) / pixel_scale.x
	...
	var footer_top: float = _scene.size.y - 144.0 - float(PixelUI.safe_bottom)
