...
Models (among others): DoT, shields, phase 2, blastAll, enemy `rfm` (hero RFE), `erb`/`erbAll`, `wipeShields`, hero `gainProtocol` (adds to track-wide reroll budget). Still omits items, summons, taunt, cloak, cower, per-turn protocol income, nudge/set, etc.
